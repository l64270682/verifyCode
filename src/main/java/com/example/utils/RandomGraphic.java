...
public class RandomGraphic {
     ...
     * 算数验证码
     */
    public String drawAlpha2(String graphicFormat, OutputStream out) throws IOException {
        this.charCount = 7;
        int x1 = randomInt(0, 99);
        int x2 = randomInt(0, 99);
        int result = x1 + x2;
        String charValue = Integer.toString(x1) + "+" + Integer.toString(x2) + "=?  ";
        draw(charValue, graphicFormat, out);
     ...
     * 排序验证码
     */
    public String drawAlpha3(String graphicFormat, OutputStream out) throws IOException {
        int x1 = (int) Math.pow(10, this.charCount);
        int result = randomInt(x1/10, x1);
        String charValue = Integer.toString(result);
        //System.out.println(charValue);
        draw(charValue, graphicFormat, out);
        String[] checked = charValue.split("");
        StringBuilder sb = new StringBuilder("");
        String ww;
        for (int i = 0; i < checked.length; i++) {
            for (int j = i ; j < checked.length; j++) {
                if (checked[i].compareTo(checked[j]) > 0) {
                    ww = checked[i];
                    checked[i] = checked[j];
                    checked[j] = ww;
                }
    ...
    public static void main(String[] args) throws Exception {
        System.out.println(RandomGraphic.createInstance(6).drawAlpha3(RandomGraphic.GRAPHIC_PNG, new FileOutputStream("E:/myimg.png")));
    }
